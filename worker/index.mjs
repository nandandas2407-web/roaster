...
function extractMetadata(html, url) {
  ...
  return { url, title: title || url, description: "Fetched", textPreview: words || "No readable content.", status: "success" };
}
  ...
  if (Array.isArray(links)) {
    const promises = links.filter(l => l && typeof l === "string" && l.trim()).slice(0, 3).map(async (rawUrl) => {
      ...
      try {
        ...
        return extractMetadata(await res.text(), url);
      } catch { return { url, title: url, description: "Timeout", textPreview: "", status: "failed" }; }
    });
